...
static long pifo_rb_update_elem(struct bpf_map *map, void *key, void *value,
			       u64 flags)
{
	return -ENOTSUPP;
}
...
/* Called from syscall or from eBPF program */
static long pifo_rb_delete_elem(struct bpf_map *map, void *key)
{
	struct bpf_pifo_rb *rb = bpf_pifo_rb(map);
	u64 rank = *(u64 *) key;
	struct xdp_frame *frm;
...
static long bpf_for_each_pifo_rb(struct bpf_map *map, bpf_callback_t callback_fn,
...
static u64 pifo_rb_map_mem_usage(const struct bpf_map *map)
{
	struct bpf_pifo_rb *rb = container_of(map, struct bpf_pifo_rb, map);
	u64 usage = sizeof(struct bpf_pifo_rb);

	usage += rb->num_queued * PAGE_SIZE; /* FIXME: not always accurate */

	return usage;
}

static long pifo_rb_map_redirect(struct bpf_map *map, u64 index, u64 flags)
{
...
const struct bpf_map_ops pifo_rb_map_ops = {
	...
	.map_mem_usage = pifo_rb_map_mem_usage,
