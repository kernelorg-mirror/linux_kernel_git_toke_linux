...
static long pifo_map_push_elem(struct bpf_map *map, void *value, u64 flags)
{
	struct bpf_pifo_map *pifo = container_of(map, struct bpf_pifo_map, map);
...
static long pifo_map_pop_elem(struct bpf_map *map, void *value)
{
	struct bpf_pifo_map *pifo = container_of(map, struct bpf_pifo_map, map);
	union bpf_pifo_item *item;
	unsigned long flags;
...
static long pifo_map_update_elem(struct bpf_map *map, void *key, void *value,
				u64 map_flags)
{
	return -EINVAL;
}

static long pifo_map_delete_elem(struct bpf_map *map, void *key)
{
	return -EINVAL;
}

static long pifo_map_peek_elem(struct bpf_map *map, void *value)
{
	return -EINVAL;
}

static u64 pifo_map_mem_usage(const struct bpf_map *map)
{
	struct bpf_pifo_map *pifo = container_of(map, struct bpf_pifo_map, map);
	u64 queue_usage = sizeof(struct bpf_pifo_queue);
	struct bpf_pifo_queue *q = pifo->q_primary;
	u64 usage = sizeof(struct bpf_pifo_map);
	u32 levels, i;

	queue_usage += q->range * sizeof(struct bpf_pifo_bucket);
	levels = __KERNEL_DIV_ROUND_UP(ilog2(q->range),
				       ilog2(BITS_PER_TYPE(long)));
	for (i = 0; i < levels; i++)
		queue_usage += (i + 1) * sizeof(unsigned long);
	queue_usage += sizeof(unsigned long) * levels;

	usage += queue_usage * 2;

	if (pifo->elem_cache) {
		u32 elem_size =
			pifo->map.value_size + sizeof(struct bpf_pifo_element);

		usage += map->max_entries * elem_size;
		usage += sizeof(void *) * map->max_entries +
			 sizeof(struct bpf_pifo_element_cache);
	} else {
		usage += pifo->num_queued * PAGE_SIZE; /* FIXME: not always accurate */
	}

	return usage;
}

static long pifo_map_redirect(struct bpf_map *map, u64 index, u64 flags)
{
#ifdef CONFIG_NET
...
const struct bpf_map_ops pifo_xdp_map_ops = {
	...
	.map_mem_usage = pifo_map_mem_usage,
