...
static int xdp_fifo_alloc_check(union bpf_attr *attr)
{
	...
	if (attr->map_extra > U32_MAX / sizeof(struct bpf_xdp_fifo_bucket))
		return -E2BIG;
...
static long xdp_fifo_update_elem(struct bpf_map *map, void *key, void *value,
			       u64 flags)
{
	return -EOPNOTSUPP;
}
...
/* Called from syscall or from eBPF program */
static long xdp_fifo_delete_elem(struct bpf_map *map, void *key)
{
	return -EOPNOTSUPP;
}
...
static u64 xdp_fifo_mem_usage(const struct bpf_map *map)
{
	struct bpf_xdp_fifo *fifo = container_of(map, struct bpf_xdp_fifo, map);
	u64 usage = sizeof(struct bpf_xdp_fifo);
	unsigned long i;

	usage += fifo->num_buckets * sizeof(struct bpf_xdp_fifo_bucket);
...
static long xdp_fifo_map_redirect(struct bpf_map *map, u64 index, u64 flags)
{
...
const struct bpf_map_ops xdp_fifo_map_ops = {
	...
	.map_mem_usage = xdp_fifo_mem_usage,
